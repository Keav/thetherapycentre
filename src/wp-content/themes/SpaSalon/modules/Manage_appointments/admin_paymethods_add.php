...
if($_POST)
{
	...
	echo '<form method=get name="payment_setting_frm" acton="'.$location.'#payment_setups">
	<input type="hidden" name="id" value="'.$_GET['id'].'"><input type="hidden" name="page" value="Appointment"><input type="hidden" name="payact" value="setting"><input type="hidden" name="msg" value="success"></form>
...
<div class='headerdivh3'>
 ...
 <div style="text-align:right; margin-top:-40px; margin-bottom:15px;"><a href="<?php echo site_url();?>/wp-admin/admin.php?page=Appointment#payment_setups" name="btnviewlisting" class="button-primary" title="<?php _e('Back to Payment Options List','templatic');?>"/><?php _e('&laquo; Back to Payment Options List','templatic'); ?></a></div>
    <p><?php _e('Edit the payment option settings','templatic');?></p>
</div>
<form action="<?php echo site_url();?>/wp-admin/admin.php?page=Appointment&payact=setting&id=<?php echo $_GET['id'];?>#payment_setups" method="post" name="payoptsetting_frm">
  <?php if($_GET['msg']){?>
