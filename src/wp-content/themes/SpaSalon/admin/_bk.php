        ...
        ?>
		  ...
		  <div class="block" id="option_update">
            ...
            <span style="display:none"><?php echo $method; ?></span>
                ...
                <p>This updater will collect a file from the templatic.com server. It will download and extract the files to your current theme&prime;s admin folder. </p>
                ...
                <span id="framework_upgrade_process_span_id" style="display:none;"><h2 style="color:#900; "><?php _e('Updating, It will take few moments. Please wait....','templatic');?></h2></span>
				<?php } else { ?>                
               <?php
            ...
            <?php } ?>
            <?php do_action('templ_module_auto_update');?>
            </div>
...
function tmpl_current_framework_version($url = ''){
	...
	if(!is_wp_error($temp_file_addr) && $file_contents = file($temp_file_addr)) {
        ...
        return __('Currently Unavailable','templatic');
    }
}
...
if($_REQUEST['page'] == 'update_framework'){
	if($_REQUEST['action']){
		$src=TEMPL_FRAMWORK_ZIP_FOLDER_PATH;
		$dest=TT_FRAMEWORK_FOLDER_PATH;
		if($_REQUEST['zip'])
		{
			$src=$_REQUEST['zip'];
		}
...
function templ_is_updated($curversion='',$liveversion='')
{
