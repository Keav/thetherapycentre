...
if(!class_exists('templ_latest_posts_with_images')){
	class templ_latest_posts_with_images extends WP_Widget {
		...
		//Constructor
		global $thumb_url;
			$widget_ops = array('classname' => 'widget special', 'description' => apply_filters('templ_latestpostwimg_widget_desc_filter','Display Posts with Image & Date') );
            ...
            <?php }elseif($post_images = bdw_get_images($post->ID)){ ?>
             <a  class="post_img" href="<?php the_permalink(); ?>">
              <?php
				...
				?>
             <img  src="<?php echo $post_images;?>" alt="<?php the_title(); ?>" title="<?php the_title(); ?>" width="50" height="50"  /> </a>
            <?php
            }?>
					
            <h4><a class="widget-title" href="<?php the_permalink(); ?>"><?php the_title(); ?></a> <br />  <span class="post_author">by <?php the_author_posts_link(); ?> at <?php the_time(templ_get_date_format()) ?>  / <?php comments_popup_link(__('No Comments','templatic'), __('1 Comment','templatic'), __('% Comments','templatic'), '', __('Comments Closed','templatic')); ?> </span></h4> 
