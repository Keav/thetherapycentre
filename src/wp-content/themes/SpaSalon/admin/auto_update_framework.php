...
function get_current_version($url = '')
{
	if(!empty($url)){
		$framework_url = $url;
	} else {
    	$framework_url = TT_FRAMEWORK_FOLDER_PATH.'change_log.txt';
    }

	$temp_file_addr = ($framework_url);
       ...
       $to = TT_FRAMEWORK_FOLDER_PATH;
        ...
        ?>
            ...
            <span style="display:none"><?php echo $method; ?></span>
                ...
                <input type="submit" class="button" value="<?php _e('Update Framework','templatic');?>" />
                </form>
				<?php } else { ?>                
               <?php
...
if($_REQUEST['page'] == 'update_framework'){
	if($_REQUEST['action'] == 'upgrade_framework' ){		
	  ...
	  $filename = TT_FRAMEWORK_FOLDER_PATH.'change_log.txt';
	$temp_file_addr = download_url($framework_url);
	if(!is_wp_error($temp_file_addr) && $file_contents = file($temp_file_addr)) {
       $file=fopen($framework_url,"r") or exit("Unable to open file!");
	 
	   $file1 = fopen($filename, "w") or die("Cannot open $file_name");
			while (!feof($file))
			  {
			 	 $a =  fgetc($file);
